...
class JobManager:
    ...
    def submit_job(
        ...
        Args:
            ...
            environment: Environment to use (env or env_py36)
        ...
        self._start_job(job_id, script_path, args, job_dir, environment)
    ...
    def _start_job(self, job_id: str, script_path: str, args: Dict, job_dir: Path, environment: str):
        ...
        def run_job():
            ...
            try:
                # Build command - use mamba run with specified environment
                env_path = f"./{environment}"
                cmd = ["mamba", "run", "-p", env_path, "python", script_path]

                # Add script arguments
                for key, value in args.items():
                    if value is not None:
                        if key == "input_files":
                            # Handle list of files (for batch processing)
                ...
                # Set output paths if not already specified
                if "output" not in args and "output_dir" not in args:
                    output_file = job_dir / "output.json"
                    cmd.extend(["--output", str(output_file)])
                elif "output_dir" in args and args["output_dir"] is None:
                    cmd.extend(["--output_dir", str(job_dir)])
    ...
    def get_job_result(self, job_id: str) -> Dict[str, Any]:
        ...
        # Load output - look for various output files
        job_dir = self.jobs_dir / job_id
        output_files = []

        # Look for JSON output
        for output_file in [job_dir / "output.json", job_dir / "result.json"]:
            if output_file.exists():
                with open(output_file) as f:
                    result = json.load(f)
                result["output_files"] = self._find_output_files(job_dir)
                return {"status": "success", "result": result}

        # Look for CSV and other outputs
        output_files = self._find_output_files(job_dir)
        if output_files:
            return {
                "status": "success",
                "result": {"output_files": output_files, "message": "Job completed successfully"}
            }
        else:
            return {"status": "error", "error": "No output files found"}

    def _find_output_files(self, job_dir: Path) -> List[str]:
        """Find all output files in the job directory."""
        output_files = []
